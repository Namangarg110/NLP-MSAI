...
def train_model(model, train_dataloader, val_dataloader, epochs=1, lr=0.001, device='cpu'):
...
def get_model(vocab_size, embed_dim, hidden_dim, out_dim, n_layers, device, weight_path=None):
    model = RNNModel(vocab_size, embed_dim, hidden_dim, out_dim, n_layers).to(device)
    if weight_path is not None:
        model.load_state_dict(torch.load(weight_path))
    return model


def get_embeddings(weight_paths):
    VOCAB_SIZE, EMBEDDING_DIM, HIDDEN_DIM, OUTPUT_DIM, N_LAYERS = get_model_parameters()

    embeddings = []
    for p in weight_paths:
        m = get_model(VOCAB_SIZE, EMBEDDING_DIM, HIDDEN_DIM, OUTPUT_DIM, N_LAYERS, 'cpu', p)
        embeddings.append(m.embedding.weight.detach().cpu().numpy())
    return embeddings
...
def get_model_parameters():
    VOCAB_SIZE = 50258 #len(tokenizer)
    EMBEDDING_DIM = 128
    HIDDEN_DIM = 256
    OUTPUT_DIM = VOCAB_SIZE
    N_LAYERS = 2
    return VOCAB_SIZE, EMBEDDING_DIM, HIDDEN_DIM, OUTPUT_DIM, N_LAYERS
...
def main():
    ...
    weight_paths = ['./RNN/model1.pth', './RNN/model2.pth']

    model1_embeddings, model2_embeddings = get_embeddings(weight_paths)
    ...
    visualize_embeddings(model1_embeddings, model2_embeddings, VOCAB_SIZE)

# Visualize embeddings with t-SNE
def visualize_embeddings(embeddings1, embeddings2, vocab_size):
